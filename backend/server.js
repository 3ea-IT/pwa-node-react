...
app.post("/forgot-password", async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "OTP sent to email. Please wait this might take 1-2 miutes.",
    });
  } catch (error) {
  ...
  try {
    ...
    if (userData.referralCode) {
      ...
      if (rows.length > 0) {
        ...
        await db
          .promise()
          .query(insertReferralReferrer, [referringUserId, null, 100, date]); // Using null for now, will update after inserting the new user
      } else {
        return res
          .status(400)
...
app.post("/update-phone", (req, res) => {
  ...
  if (!oldMob || !newMob) {
    return res
      .status(400)
      .json({ message: "Old and new mobile numbers are required" });
  }
  ...
  if (temporaryUserData[oldMob]) {
    ...
    res.status(200).json({
      message: "Phone number updated and new OTP sent successfully",
      otp,
    });
  } else {
...
app.post("/login", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Authentication successful", token, user_id: user.id });
  } catch (error) {
...
app.post("/subscribe", (req, res) => {
  ...
  const query =
    "INSERT INTO pushsubs (user_id, ENDPOINT, KEYID, TOKEN, TIME) VALUES (?, ?, ?, ?, ?)";
...
app.get("/api/fetch-fit-data", async (req, res) => {
  ...
  if (!startTimeNs || !endTimeNs) {
    return res
      .status(400)
      .json({ message: "startTimeNs and endTimeNs are required" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching fitness data", error: error.message });
  }
});
...
app.get("/user-points/:userId", verifyJWT, (req, res) => {
  ...
  db.query(getUserPoints, [userId], (err, results) => {
    if (err) {
      ...
      res
        .status(500)
        .json({ message: "Error fetching user points", error: err });
    } else {
...
app.get("/recent-invites/:referralCode", (req, res) => {
  ...
  db.query(query, [referralCode], (error, results) => {
    if (error) {
      res
        .status(500)
        .json({ message: "Error fetching recent invites with points" });
    } else {
      res.status(200).json(results);
    }
  });
...
app.get("/invited-friends/:referralCode", (req, res) => {
  ...
  db.query(query, [referralCode], (error, results) => {
    if (error) {
      res
        .status(500)
        .json({ message: "Error fetching invited friends with points" });
    } else {
      res.status(200).json(results);
    }
  });
...
app.post("/save-answer", verifyJWT, (req, res) => {
  ...
  db.query(
    insertAnswer,
    [req.userId, ques_id, ans, is_correct, quiz_id, date],
    (err, result) => {
      if (err) {
...
app.post("/save-quiz-result", verifyJWT, (req, res) => {
  ...
  db.query(insertResult, [req.userId, score, points, date], (err, result) => {
    if (err) {
      ...
      db.query(
        insertPointsLog,
        [req.userId, points, "cr", pointsLogRemark, "PEARN", date],
        (err, logResult) => {
          if (err) {
            res
              .status(500)
              .json({ message: "Error saving points log", error: err });
          } else {
            // Fetch the current wallet points
            db.query(getUserWallet, [req.userId], (err, walletResult) => {
              if (err) {
                res.status(500).json({
                  message: "Error fetching wallet points",
                  error: err,
                });
              } else {
                ...
                db.query(
                  updateUserWallet,
                  [updatedWalletPoints, req.userId],
                  (err, updateResult) => {
                    if (err) {
                      res.status(500).json({
                        message: "Error updating wallet points",
                        error: err,
                      });
                    } else {
                      res.status(200).json({
                        message:
                          "Quiz result, points log, and wallet updated successfully",
                      });
                    }
                  }
                );
...
app.get("/medical-questions", verifyJWT, (req, res) => {
  ...
  db.query(query, (error, results) => {
    if (error) {
      res
        .status(500)
        .json({ message: "Error fetching medical questions", error });
    } else {
      res.status(200).json(results);
    }
  });
...
app.post("/save-kyc", verifyJWT, (req, res) => {
  ...
  const updateQueryBasic =
    "UPDATE kyc SET pincode = ?, area = ?, city = ?, state = ?, country = ? WHERE user_id = ?";
  const updateQueryVitals =
    "UPDATE kyc SET gender = ?, height = ?, weight = ?, blood_group = ?, age = ? WHERE user_id = ?";
  const insertQuery =
    "INSERT INTO kyc (user_id, pincode, area, city, state, country, gender, height, weight, blood_group, age) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

  const logQuery =
    "INSERT INTO kyc_logs (user_id, type, remark, date) VALUES (?, ?, ?, ?)";
  const vitalsQuery =
    "INSERT INTO vitals (user_id, weight, date) VALUES (?, ?, ?)";
    ...
    if (err) {
      ...
      if (pincode || area || city || state || country) {
        ...
        db.query(updateQueryBasic, updateValues, (err, updateResult) => {
          if (err) {
            ...
            res
              .status(500)
              .json({ message: "Error updating KYC data", error: err });
          } else {
            const fieldsToUpdate = { pincode, area, city, state, country };
            Object.keys(fieldsToUpdate).forEach((field) => {
              if (
                fieldsToUpdate[field] &&
                fieldsToUpdate[field] !== existingData[field]
              ) {
                db.query(
                  logQuery,
                  [
                    userId,
                    field,
                    existingData[field] ? "updated" : "added",
                    date,
                  ],
                  (logErr) => {
                    if (logErr) {
                      console.error(
                        `Error updating KYC log for ${field}:`,
                        logErr
                      );
                    }
                  }
            ...
            res
              .status(200)
              .json({ message: "KYC basic data updated successfully" });
          }
        });
      }
      ...
      if (gender || height || weight || bloodGroup || age) {
        ...
        db.query(updateQueryVitals, updateValues, (err, updateResult) => {
          if (err) {
            ...
            res
              .status(200)
              .json({ message: "KYC vitals data updated successfully" });
          }
        });
      }
    } else {
      ...
      db.query(insertQuery, insertValues, (err, insertResult) => {
        if (err) {
          ...
          res
            .status(500)
            .json({ message: "Error inserting KYC data", error: err });
        } else {
          ...
          Object.keys(fieldsToInsert).forEach((field) => {
            if (fieldsToInsert[field]) {
              db.query(logQuery, [userId, field, "added", date], (logErr) => {
                if (logErr) {
                  console.error(
                    `Error logging KYC insertion for ${field}:`,
                    logErr
                  );
                }
              });
...
app.get("/medicine-logs", (req, res) => {
  ...
  const query =
    "SELECT ml.*, m.med_name FROM Medicine_logs ml JOIN Medicine m ON ml.med_id = m.id WHERE ml.user_id = ?";
  db.query(query, [userId], (error, results) => {
    if (error) {
...
app.post("/medicine-logs", (req, res) => {
  ...
  const query =
    "INSERT INTO Medicine_logs (user_id, med_id, dosage, time_remind, days) VALUES (?, ?, ?, ?, ?)";
  db.query(
    query,
    [user_id, med_id, dosage, JSON.stringify(time_remind), days],
    (error, results) => {
      if (error) {
