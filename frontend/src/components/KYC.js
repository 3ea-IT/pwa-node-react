...
const KYC = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}kyc/${userId}`)
      .then((response) => {
        if (response.data) {
          setFormData({
    ...
    axios
      .get(`${process.env.REACT_APP_API_URL}medical-questions`, {
  ...
  const handleNext = () => {
    ...
    if (activeTab === "Basic") {
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}save-kyc`, dataToSend, config)
        .then((response) => {
          console.log("Basic KYC data saved successfully:", response.data);
          setActiveTab("Vitals");
        })
        .catch((error) => {
          console.error("Error saving Basic KYC data:", error);
        });
      ...
      axios
        .post(`${process.env.REACT_APP_API_URL}save-kyc`, dataToSend, config)
        .then((response) => {
          console.log("Vitals KYC data saved successfully:", response.data);
          setActiveTab("Medical");
        })
        .catch((error) => {
          console.error("Error saving Vitals KYC data:", error);
        });
      ...
      axios
        .post(
          `${process.env.REACT_APP_API_URL}save-medical-answers`,
          { userId, answers: [currentAnswer] },
          config
        )
        .then((response) => {
  ...
  const renderVitalsTab = () => (
    <div className="vitals-tab">
      <p>
        Your individual parameters are important for in-depth personalization.
      </p>
        ...
        <>
          ...
          <div className="dropdown-container">
            ...
            >
              ...
              {medicalQuestions[currentQuestion].option1 && (
                <option value="1">
                  {medicalQuestions[currentQuestion].option1}
                </option>
              )}
              {medicalQuestions[currentQuestion].option2 && (
                <option value="2">
                  {medicalQuestions[currentQuestion].option2}
                </option>
              )}
              {medicalQuestions[currentQuestion].option3 && (
                <option value="3">
                  {medicalQuestions[currentQuestion].option3}
                </option>
              )}
              {medicalQuestions[currentQuestion].option4 && (
                <option value="4">
                  {medicalQuestions[currentQuestion].option4}
                </option>
              )}
              {medicalQuestions[currentQuestion].option5 && (
                <option value="5">
                  {medicalQuestions[currentQuestion].option5}
                </option>
              )}
          ...
          <div className="progress-indicator">
            <span>
              {currentQuestion + 1}/{medicalQuestions.length}
            </span>
          </div>
        </>
      )}
      ...
      {!isCompleted ? (
        <>
          ...
          <button className="next-button" onClick={handleNext}>
            {activeTab === "Medical" &&
            currentQuestion === medicalQuestions.length - 1
              ? "Submit"
              : "NEXT"}
          </button>
        </>
